...
def Quiz(flashcards_df):
    ...
    if flashcards_df is not None:
        if not flashcards_df.empty:
            col1, col2, col3 = columns([1, 1, 1], gap="small")
            # Check the shuffle toggle
            shuffle_enabled = col3.toggle(
                "Shuffle flashcards", 
            ...
            Show_all_anwsers = col2.toggle(
                "Learning Mode", 
                key="show_all_anwsers", 
                value=session_state.Show_all_anwsers, 
                help="Show all answers"
            )
            auto_continue = col1.toggle(
                "Auto Continue", 
                key="auto_continue", 
                value=session_state.auto_continue, 
                help="Automatically continue to the next flashcard after answering",
            )
            ...
            with container(border=True):
                ...
                # Horizontal Buttons
                left_button, right_button, _, correct, wrong, show_button = columns([1,1,1,1,1,2], gap="small")  # Create space for buttons
                with left_button:
                ...
                with wrong:
                    if button("❌", key="wrong_button", use_container_width=True, type="primary" if any(result[0] == flashcard['English'] and not result[2] for result in session_state.Results) else "secondary"):
                        # Remove any previous entry for this flashcard before adding the new one
                        session_state.Results = [result for result in session_state.Results if result[1] != flashcard['English']]
                ...
                if Show_all_anwsers:
                    ...
                    try:
                        ...
                        if session_state.auto_continue:
                            sleep(MP3(audio_path).info.length*5)
                            session_state.flashcard_index = (current_index + 1) % total_flashcards
                            rerun()
            ...
            if not Show_all_anwsers:
                with expander(
                    ...
                    f" - ▶️ Not Answered ({sum(r[3] is None for r in session_state.Results) / l * 100:.1f}%) ",
                    #expanded=True
                ):
            ...
            _, message_col, _ = columns([1, 2, 1], gap="small")  # Center the no flashcards message
            with message_col:
