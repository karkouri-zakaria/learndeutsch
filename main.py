...
def main():
    ...
    if session_state.flashcards_df is not None:
        with sidebar:
            ...
            session_state.Show_all_anwsers = col2.toggle("ℹ️", key="show", value=session_state.Show_all_anwsers, help="Show all answers")
            session_state.shuffle_enabled = col3.toggle("🔀", key="shuffle", value=session_state.shuffle_enabled, help="Randomize the flashcards order")
    if session_state.Results and len(session_state.Results) > 0 and session_state.flashcards_df is not None:
        with sidebar:
            col1, col2=columns(2, gap="small", vertical_alignment="center")
            col1.metric(
            f"Score:",
        ...
        col2.button("Reset", on_click=lambda: session_state.Results.clear(), icon="🧹", use_container_width=True)
