...
def save_results():
    with open(Path(f"./Results_{session_state.uploaded_file_data.name[:-5]}.csv"), 'w', newline='', encoding='utf-8-sig') as f:
        writer(f).writerows([["English", "Deutsch", "index"]] + [[r[1], r[2], r[0]] for r in session_state.Results if not r[3]])
@cache_data
